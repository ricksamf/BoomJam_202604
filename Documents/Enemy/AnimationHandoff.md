...
> **当前最终态**(2026-05-14):三个 ABP 都是最简单的单 Slot 结构。**Slot 名按敌人类型不同**:Melee 是 `UpperBody`、Pistol/MG 是 `DefaultSlot`。原因看 §2.1.c 历次踩坑记录。

### 2.1.a AnimGraph(Melee)

```
[Output Pose] ── Result ◄── Pose [Slot 'UpperBody'] ── Source ◄── Pose [StateMachine 'Main States']
```

- Slot 名:**`UpperBody`**(对应 `AS_Enemy_slash_Montage` 的 SlotName)
- 美术 Slash Montage 是 Full Body(`additive_anim_type = AAT_NONE`),Slot 直接吐 Montage 全身 pose 给 Output。Locomotion 在 Montage 播放期间被全身覆盖

### 2.1.b AnimGraph(Pistol / MachineGun)

```
[Output Pose] ── Result ◄── Pose [Slot 'DefaultSlot'] ── Source ◄── Pose [StateMachine 'Main States']
```

- Slot 名:**`DefaultSlot`**(对应美术新 Montage `AS_EnemyPistolShoot_Montage` / `AS_EnemyRifleShoot_Montage` 的 SlotName)
- 美术新 Montage 是 Full Body,远程敌人开火时**全身覆盖 Locomotion**——下半身不再保持站姿/移动,跟攻击 Montage 一起做 Fire 动作。这是当前妥协(用户原话:"不做上半身射击也行")。Pistol 是站桩射手 / MG 攻击期间也基本不动,全身 Montage 视觉上影响很小

### 2.1.c Slot Name 历次踩坑

UE 路由规则:`PlayAnimMontage(M)` 把 `M` 路由到 ABP 里**和 `M` 自身 SlotName 同名的 Slot 节点**。对不上就静默播完(Montage 时间轴推进,屏幕上没动画)。

走过的弯路:
- v1:三个 ABP Slot 都叫 `UpperBody`,对 Melee 工作(Montage SlotName=`UpperBody`),但美术 Pistol/MG Montage SlotName 是 `DefaultSlot`,不匹配 → 播不出
- v2:误以为 Pistol/MG 的不正常是 Additive 问题,加了 `ApplyMeshSpaceAdditive`(因为初版用的 `MM_Pistol_Fire` / `MM_Rifle_Fire` 是 Mesh Space Additive)→ 改 ABP 加了节点也不行
- v3(当前):根因是 Slot 名不匹配。Pistol/MG Slot 节点改名 `DefaultSlot`,移除 `ApplyMeshSpaceAdditive`(Full Body Montage 不需要)→ 三个敌人都能播

**给后接手者**:Montage 编辑器里能选的 SlotName 列表来自 **Skeleton 资产的 Anim Slot Manager**(Skeleton 资产 → `Window → Anim Slot Manager`)。如果只看到 `DefaultSlot`,说明 Skeleton 里只注册了 `DefaultSlot`,要用其他名字得先去 Skeleton 资产里加。这个项目 `SK_Mannequin` 里默认注册了 `DefaultSlot` 和 `UpperBody`(因为示例 `ABP_Unarmed` 用了 `UpperBody`),美术拿默认是 `DefaultSlot`。
...
每个挂给敌人的 Montage:
- **Target Skeleton**:`SK_Mannequin`(或通过 §5 的 Compatible Skeletons 兼容)
- **SlotName**:看敌人类型——
  - Melee 用 `UpperBody`(对应 `ABP_MeleeEnemy` 的 Slot 节点)
  - Pistol / MG 用 `DefaultSlot`(对应 `ABP_PistolEnemy` / `ABP_MachineGunEnemy` 的 Slot 节点)
- **Additive 类型**:推荐 `AAT_NONE`(Full Body)。当前三个 Montage 都是 Full Body。如果以后给 Additive,需要回到 §2 把 ABP 加上 `ApplyMeshSpaceAdditive` / `ApplyAdditive` 节点(本次 v2 走过这条路又退回来,看 §2.1.c)

怎么改 SlotName:
1. 双击 Montage,左侧看时间轴 Slot 一栏
2. 点 Slot 名 → 下拉 → 选 `DefaultSlot` / `UpperBody`(Skeleton 注册过的才会出现在下拉里)
3. 列表里没看到想要的名字就到 Skeleton 资产 → `Window → Anim Slot Manager` 加

### 6.3 当前实际挂的 Montage(快照)

记录当前挂法,便于排错对照(美术后续替换后请同步):

| DataAsset | 字段 | Montage | 源 Sequence | SlotName | Additive | 长度 |
|---|---|---|---|---|---|---|
| `DA_MeleeEnemy_Default` | `AttackMontage` | `/Game/Art/AnimClip/Enemy/AS_Enemy_slash_Montage` | `AS_Enemy_slash` | `UpperBody` | None (Full) | 1.5s |
| `DA_PistolEnemy_Default` | `FireMontage` | `/Game/Art/AnimClip/Enemy/AS_EnemyPistolShoot_Montage` | `AS_EnemyPistolShoot` | `DefaultSlot` | None (Full) | 1.17s |
| `DA_MachineGunEnemy_Default` | `BurstMontage` | `/Game/Art/AnimClip/Enemy/AS_EnemyRifleShoot_Montage` | `AS_EnemyRifleShoot` | `DefaultSlot` | None (Full) | 0.2s |

注意:MG 的 Montage 只有 0.2s 但 `FireRate ≈ 6/s`(每发间隔 ~0.17s),意味着 Burst 期间几乎每发都会重启 Montage,视觉是连续 recoil。如果美术给的 Montage 时长大于 1/FireRate,建议在 `AMachineGunEnemy::FireOneBullet` 里加"本 Burst 内只播一次"的 flag(本版未做)。

### 6.4 从 AnimSequence 创建 Montage
...
4. 打开该 Montage,按 §6.2 把 SlotName 设成对应敌人的(Melee=`UpperBody`,Pistol/MG=`DefaultSlot`)
...
| Montage | 触发点 | 频率 | 注意 |
|---|---|---|---|
| `AttackMontage`(Melee) | `FEnemyMeleeSwingTask::EnterState` | 每次 Swing 一次(LockOn → Dash → Swing 序列里 Swing 开始帧) | Montage 时长建议 ≤ `SwingDuration`(默认 0.4s);太长会被下一次 Swing 打断 |
| `FireMontage`(Pistol) | `APistolEnemy::FireProjectile` | 每次开枪一次(Aim → Fire 序列的 Fire 帧) | 时长建议 ≤ `Cooldown`(默认 1.6s) |
| `BurstMontage`(MG) | `AMachineGunEnemy::FireOneBullet` | **每颗子弹一次**(Burst 期间 ~6/s) | 重复调用会从头重播。建议 Montage 设计成短 recoil pulse(≤ 1/FireRate);如果美术给的更长,在 `FireOneBullet` 加 bool flag 控制只播一次 |

### 6.6 三个 Montage 都可留空

`PlayAttackMontage` 里先 `if (!Data || !Data->XxxMontage) return;` — 字段留空时静默跳过,不崩。适合增量配置(先挂 Melee,Pistol/MG 后续再做)。
